...
const struct test_s test = {
    .gyro_sens      = 32768/250,
    .accel_sens     = 32768/2,  //FSR = +-2G
    .reg_rate_div   = 0,    /* 1kHz. */
    .reg_lpf        = 3,    /* 42Hz low pass filter*/
    .reg_gyro_fsr   = 0x00,    /* 250dps. 0x00 0x08 0x10 0x18 */
    .reg_accel_fsr  = 0x00,  /* Accel FSR setting = 2g.  0x00 0x08 0x10 0x18*/
    .wait_ms        = 200,   //200ms stabilization time
    .packet_thresh  = 200,    /* 200 samples */
    .min_dps        = 20.f,  //Must exceed 20 dps for Gyro Criteria C
    .max_dps        = 105.f, //Cannot exceed 60 dps threshold for Gyro Criteria B
...
int mpu_init(struct int_param_s *int_param)
{
    ...
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, data))
		...
		/* Set to SPI mode */
		data[0] = BIT_I2C_IF_DIS | BIT_I2C_MST_EN;
    if (i2c_write(st.hw->addr, st.reg->user_ctrl, 1, data))//使用IIC模式时请注释掉这一段
        return -1;
   ...
   st.chip_cfg.accel_half = 0;
    ...
    if (mpu_set_gyro_fsr(2000))//250 500 1000 2000
        return -1;
    if (mpu_set_accel_fsr(16))//2 4 8 16
        return -1;
    if (mpu_set_lpf(42))//设置采样率函数末尾会更新lpf 这里设置无效
        return -1;
...
int mpu_set_bypass(unsigned char bypass_on)
{
    ...
    //if (st.chip_cfg.bypass_mode == bypass_on)//SPI模式下 不要直接返回 有可能改动过user_ctrl但没有置bypass_on 重新初始化以下语句来保证正常操作
        //return 0;
...
{
    unsigned char tmp[6];
    unsigned char tries = 20;
    int result = 0x07;
    short data;

    mpu_set_bypass(0);//SPI模式这里等于0 之前有对user_control寄存器操作 这里必须重新使能i2c_master
	
    tmp[0] = AKM_POWER_DOWN;
    if (MPU9250_AK8963_SPIx_Writes(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, tmp))//i2c_write(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, tmp)
        return 0x07;
		delay_ms(1);
    tmp[0] = AKM_BIT_SELF_TEST;
    if (MPU9250_AK8963_SPIx_Writes(st.chip_cfg.compass_addr, AKM_REG_ASTC, 1, tmp))//i2c_write(st.chip_cfg.compass_addr, AKM_REG_ASTC, 1, tmp)
        goto AKM_restore;
    ...
    do {
        delay_ms(10);
        if (MPU9250_AK8963_SPIx_Reads(st.chip_cfg.compass_addr, AKM_REG_ST1, 1, tmp))//i2c_read(st.chip_cfg.compass_addr, AKM_REG_ST1, 1, tmp)
            goto AKM_restore;
    ...
    if (MPU9250_AK8963_SPIx_Reads(st.chip_cfg.compass_addr, AKM_REG_HXL, 6, tmp))//i2c_read(st.chip_cfg.compass_addr, AKM_REG_HXL, 6, tmp)
        goto AKM_restore;
...
AKM_restore:
    ...
    MPU9250_AK8963_SPIx_Writes(st.chip_cfg.compass_addr, AKM_REG_ASTC, 1, tmp);//i2c_write(st.chip_cfg.compass_addr, AKM_REG_ASTC, 1, tmp);
    tmp[0] = SUPPORTS_AK89xx_HIGH_SENS;
    MPU9250_AK8963_SPIx_Writes(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, tmp);//i2c_write(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, tmp);
    mpu_set_bypass(0);
    return result;
}
#endif
...
static int setup_compass(void)
{
    unsigned char data[4], akm_addr=0x0C;
    int result;
    ...
    for (akm_addr = 0x0C; akm_addr <= 0x0F; akm_addr++) {
        result = MPU9250_AK8963_SPIx_Reads(akm_addr, AKM_REG_WHOAMI, 1, data);//i2c_read(akm_addr, AKM_REG_WHOAMI, 1, data)
    ...
    if (MPU9250_AK8963_SPIx_Writes(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, data))//i2c_write(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, data)
        return -1;
    delay_ms(1);
    ...
    if (MPU9250_AK8963_SPIx_Reads(st.chip_cfg.compass_addr, AKM_REG_ASAX, 3, data))//i2c_read(st.chip_cfg.compass_addr, AKM_REG_ASAX, 3, data)
        return -1;
    ...
    //mpu_set_bypass(0);//SPI模式不需要此语句
		
	  /* Set up master mode, master clock, and ES bit. */
    data[0] = 0x4D;//0x4D=400khz模式 原0x40
    if (i2c_write(st.hw->addr, st.reg->i2c_mst, 1, data))
        return -1;
...
#ifdef AK89xx_BYPASS
    if (MPU9250_AK8963_SPIx_Reads(st.chip_cfg.compass_addr, AKM_REG_ST1, 8, tmp))//i2c_read(st.chip_cfg.compass_addr, AKM_REG_ST1, 8, tmp)
        return -1;
    tmp[8] = AKM_SINGLE_MEASUREMENT;
    if (MPU9250_AK8963_SPIx_Writes(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, tmp+8))//i2c_write(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, tmp+8)
        return -1;
#else
...
u8 run_self_test(void)
{
	...
	if (result == 0x7) 
	{
		...
		*/
		...
		accel[0] =0;//*= accel_sens  这里直接采用加速度计绝对水平位置
		accel[1] =0;//*= accel_sens
		accel[2] =0;//*= accel_sens
...
u8 mpu_dmp_get_data(float *pitch,float *roll,float *yaw)
{
	...
	**/
		...
		Gyo_Raw[0][1]=Gyo_Raw[0][0];
		Gyo_Raw[1][1]=Gyo_Raw[1][0];
		Gyo_Raw[2][1]=Gyo_Raw[2][0];
		
		Acc_Raw[0][1]=Acc_Raw[0][0];
		Acc_Raw[1][1]=Acc_Raw[1][0];
		Acc_Raw[2][1]=Acc_Raw[2][0];
		
		Mag_Raw[0][1]=Mag_Raw[0][0];
		Mag_Raw[1][1]=Mag_Raw[1][0];
		Mag_Raw[2][1]=Mag_Raw[2][0];
		...
		Mag_Raw[0][0]=compass[0];
		Mag_Raw[1][0]=compass[1];
		Mag_Raw[2][0]=compass[2];
	...
	if(sensors&INV_WXYZ_QUAT) 
	{
		...
		*/
		*pitch = asin(2*q2*q3+2*q0*q1)* 57.3;	// pitch
		*roll  = atan2(2*q0*q2-2*q1*q3,2*q0*q0+2*q3*q3-1)* 57.3;// roll
		*yaw   = atan2(2*q0*q3-2*q1*q2,2*q0*q0+2*q2*q2-1) * 57.3;	//yaw
...
u8 mpu_mpl_get_data(float *pitch,float *roll,float *yaw)
{
		...
		if(sensors&INV_WXYZ_QUAT) 
	{
		...
		DMP_Pitch = asin(2*q2*q3+2*q0*q1) * 57.3;
		DMP_Roll = atan2(2*q0*q2-2*q1*q3,2*q0*q0+2*q3*q3-1) * 57.3;
		DMP_Yaw = atan2(2*q0*q3-2*q1*q2,2*q0*q0+2*q2*q2-1) * 57.3;
		...
		Acc_Raw[0][0]=accel_short[0];
		Acc_Raw[1][0]=accel_short[1];
		Acc_Raw[2][0]=accel_short[2];
		
		Mag_Raw[0][0]=compass_short[0];
		Mag_Raw[1][0]=compass_short[1];
		Mag_Raw[2][0]=compass_short[2];
	}
		*pitch = (data[0]/q16);
		*roll  = (data[1]/q16);
    *yaw   = (data[2]/q16);
